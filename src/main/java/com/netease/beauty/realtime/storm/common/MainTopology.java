...
import com.netease.beauty.realtime.storm.bilog.BiLogSplittingBolt;
import com.netease.beauty.realtime.storm.bilog.ReadBolt;
import com.netease.beauty.realtime.storm.dalog.DaLogExposeBolt;
import com.netease.beauty.realtime.storm.dalog.DaLogSplittingBolt;
...
public class MainTopology {
	...
	public static void main(String[] args) throws Exception {
		...
		KafkaHelper.getKafkaTopicsAsArray(envMap);//初始化KafkaHelper的tps变量
		//创建拓扑
	...
	private static TopologyBuilder buildTopologyBuilder() {
		 ...
		 * bi拓扑
		 */
		tb.setBolt(StormConstants.BILOG_SPLITTING_BOLT, new BiLogSplittingBolt()).shuffleGrouping(StormConstants.MAIN_SPOUT, KafkaHelper.tps[0]);//根据topic分组，bi日志
		tb.setBolt(StormConstants.READ_BOLT, new ReadBolt()).shuffleGrouping(StormConstants.BILOG_SPLITTING_BOLT, StormConstants.READ);
		
		/**
		 * da拓扑
		 */
		tb.setBolt(StormConstants.DALOG_SPLITTING_BOLT, new DaLogSplittingBolt()).shuffleGrouping(StormConstants.MAIN_SPOUT, KafkaHelper.tps[1]);//根据topic分组，da日志
		tb.setBolt(StormConstants.DALOG_EXPOSE_BOLT, new DaLogExposeBolt()).shuffleGrouping(StormConstants.DALOG_SPLITTING_BOLT, DaConstants.EXPOSE);
		return tb;
	}
