...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    private synchronized void initPool(List<HostAndPort> masters) {
		...
		public void run() {
			...
			try {
			    jedis.subscribe(new JedisPubSubAdapter() {
					@Override
					public void onMessage(String channel, String message) {
						//mesg format : <master name> <oldip> <oldport> <newip> <newport>
   ...
   public static void main(String[] args) {
	   String DELIMETER=";";
		List<String> masterList = BaseUtils.splitToList("m1;m2",DELIMETER);
		Set<String> sentinelSet = new HashSet<String>(BaseUtils.splitToList("music17.photo.163.org:26382;music17.photo.163.org:26380;music17.photo.163.org:26379", DELIMETER));
		GenericObjectPoolConfig config = new GenericObjectPoolConfig();
		config.setMaxTotal(10);
		config.setMaxIdle(10);
		config.setMinIdle(0);
		config.setTestOnBorrow(true);
		config.setMinEvictableIdleTimeMillis(15000);//15 seconds.
		config.setTimeBetweenEvictionRunsMillis(10000); //10 seconds.
		config.setNumTestsPerEvictionRun(10);
		config.setTestWhileIdle(true);
		 ShardedJedisSentinelPool poll = new ShardedJedisSentinelPool(masterList, sentinelSet, config, 100000);
		 poll.getResource().set("aaa", "bbbb");
		 poll.getResource().set("bbxxx", "bbbb");
		 System.out.println(poll.getResource().get("aaa"));
		 try {
			Thread.sleep(100000);
		} catch (InterruptedException e) {
